...
def upgrade():
    ...
    # set password to pwd1234
    op.execute(
        """update "user" set _password = '$2b$12$oJ5CDLQ6LPluUJKC6fDGxORf4lXnlHtmjsRKq3XmoJZPRbzpBOWhO'"""
    )
    op.execute("""update "user" set email = 'default@email.com' where email is NULL""")
    op.execute(
        """update "user" set last_name = 'default_last_name' where last_name is NULL"""
    )
    op.execute(
        """update "user" set first_name = 'default_first_name' where first_name is NULL"""
    )
    op.execute(
